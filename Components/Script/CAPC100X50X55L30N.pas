...
Procedure CreateComponentCAPC100X50X55L30N(Zero : integer);
Var
...
Begin
    Try
        ...
        NewPcbLibComp := CreateAComponent('CAPC100X50X55L30N');
        NewPcbLibComp.Name := 'CAPC100X50X55L30N';
        NewPCBLibComp.Description := 'Capacitor, Non-polarized, Chip; 1.00 mm L X 0.50 mm W X 0.55 mm H Body';
        NewPCBLibComp.Height := MMsToCoord(0.55);

        CreateSMDComponentPad(NewPCBLibComp, '1', eTopLayer, -0.437, 0, 0, 0, eRoundedRectangular, 0.625, 0.58, 180, 50, 0, 0, True, True);
        CreateSMDComponentPad(NewPCBLibComp, '2', eTopLayer, 0.437, 0, 0, 0, eRoundedRectangular, 0.625, 0.58, 0, 50, 0, 0, True, True);

        CreateComponentTrack(NewPCBLibComp, -0.2, 0.25, -0.2, -0.25, eMechanical5, 0.025, False);
        CreateComponentTrack(NewPCBLibComp, -0.2, -0.25, -0.5, -0.25, eMechanical5, 0.025, False);
        CreateComponentTrack(NewPCBLibComp, -0.5, -0.25, -0.5, 0.25, eMechanical5, 0.025, False);
        CreateComponentTrack(NewPCBLibComp, -0.5, 0.25, -0.2, 0.25, eMechanical5, 0.025, False);
        CreateComponentTrack(NewPCBLibComp, 0.2, -0.25, 0.2, 0.25, eMechanical5, 0.025, False);
        CreateComponentTrack(NewPCBLibComp, 0.2, 0.25, 0.5, 0.25, eMechanical5, 0.025, False);
        CreateComponentTrack(NewPCBLibComp, 0.5, 0.25, 0.5, -0.25, eMechanical5, 0.025, False);
        CreateComponentTrack(NewPCBLibComp, 0.5, -0.25, 0.2, -0.25, eMechanical5, 0.025, False);
        ...
        CreateComponentTrack(NewPCBLibComp, 0.5, 0.25, 0.5, -0.25, eMechanical5, 0.025, False);
        CreateComponentTrack(NewPCBLibComp, 0.5, -0.25, -0.5, -0.25, eMechanical5, 0.025, False);
        CreateComponentTrack(NewPCBLibComp, -0.525, -0.275, -0.525, 0.275, eMechanical3, 0.12, False);
        CreateComponentTrack(NewPCBLibComp, -0.525, 0.275, 0.525, 0.275, eMechanical3, 0.12, False);
        CreateComponentTrack(NewPCBLibComp, 0.525, 0.275, 0.525, -0.275, eMechanical3, 0.12, False);
        CreateComponentTrack(NewPCBLibComp, 0.525, -0.275, -0.525, -0.275, eMechanical3, 0.12, False);
        CreateComponentTrack(NewPCBLibComp, -0.9, -0.425, -0.9, 0.425, eMechanical7, 0.05, False);
        CreateComponentTrack(NewPCBLibComp, -0.9, 0.425, 0.9, 0.425, eMechanical7, 0.05, False);
        CreateComponentTrack(NewPCBLibComp, 0.9, 0.425, 0.9, -0.425, eMechanical7, 0.05, False);
        CreateComponentTrack(NewPCBLibComp, 0.9, -0.425, -0.9, -0.425, eMechanical7, 0.05, False);
        CreateComponentArc(NewPCBLibComp, 0, 0, 0.1595, 0, 360, eMechanical7, 0.05, False);
        CreateComponentTrack(NewPCBLibComp, 0.212, 0, -0.212, 0, eMechanical7, 0.05, False);
        CreateComponentTrack(NewPCBLibComp, 0, 0.212, 0, -0.212, eMechanical7, 0.05, False);

        STEPmodel := PcbServer.PCBObjectFactory(eComponentBodyObject, eNoDimension, eCreate_Default);
        Model := STEPmodel.ModelFactory_FromFilename('C:\Users\ivans\Desktop\FESB Racing\Altium\Hardware\Components\Script\CAPC100X50X55L30.STEP', false);
...
Begin
    ...
    CreateComponentCAPC100X50X55L30N(0);
