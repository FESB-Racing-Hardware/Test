...
Procedure CreateComponentVARRBO750W80L1350T640H1650(Zero : integer);
Var
...
Begin
    Try
        ...
        NewPcbLibComp := CreateAComponent('VARRBO750W80L1350T640H1650');
        NewPcbLibComp.Name := 'VARRBO750W80L1350T640H1650';
        NewPCBLibComp.Description := 'Varistor, Radial (Disk with Offset Leads), 7.50 mm Pitch X 0.80 mm W X 13.50 mm L X 6.40 mm T X 16.50 mm H body';
        NewPCBLibComp.Height := MMsToCoord(16.5);

        CreateTHComponentPad(NewPCBLibComp, '1', eRoundHole, 1.05, 0, eBottomLayer, -3.75, 1.1, 0, 0, eRounded, 1.58, 1.58, eRounded, 1.58, 1.58, eRounded, 1.58, 1.58, 0, 0, -1.58, 0, True);
        CreateTHComponentPad(NewPCBLibComp, '2', eRoundHole, 1.05, 0, eBottomLayer, 3.75, -1.1, 0, 0, eRounded, 1.58, 1.58, eRounded, 1.58, 1.58, eRounded, 1.58, 1.58, 0, 0, -1.58, 0, True);

        CreateComponentArc(NewPCBLibComp, -3.75, 1.1, 0.425, 0, 360, eMechanical5, 0.025, False);
        CreateComponentArc(NewPCBLibComp, 3.75, -1.1, 0.425, 0, 360, eMechanical5, 0.025, False);
        CreateComponentTrack(NewPCBLibComp, -6.75, -3.2, -6.75, 3.2, eMechanical7, 0.025, False);
        CreateComponentTrack(NewPCBLibComp, -6.75, 3.2, 6.75, 3.2, eMechanical7, 0.025, False);
        CreateComponentTrack(NewPCBLibComp, 6.75, 3.2, 6.75, -3.2, eMechanical7, 0.025, False);
        CreateComponentTrack(NewPCBLibComp, 6.75, -3.2, -6.75, -3.2, eMechanical7, 0.025, False);
        CreateComponentTrack(NewPCBLibComp, -6.75, -3.2, -6.75, 3.2, eMechanical3, 0.12, False);
        CreateComponentTrack(NewPCBLibComp, -6.75, 3.2, 6.75, 3.2, eMechanical3, 0.12, False);
        CreateComponentTrack(NewPCBLibComp, 6.75, 3.2, 6.75, -3.2, eMechanical3, 0.12, False);
        CreateComponentTrack(NewPCBLibComp, 6.75, -3.2, -6.75, -3.2, eMechanical3, 0.12, False);
        CreateComponentTrack(NewPCBLibComp, -6.75, -3.2, -6.75, 3.2, eTopOverlay, 0.12, False);
        CreateComponentTrack(NewPCBLibComp, -6.75, 3.2, 6.75, 3.2, eTopOverlay, 0.12, False);
        CreateComponentTrack(NewPCBLibComp, 6.75, 3.2, 6.75, -3.2, eTopOverlay, 0.12, False);
        CreateComponentTrack(NewPCBLibComp, 6.75, -3.2, -6.75, -3.2, eTopOverlay, 0.12, False);
        ...
        CreateComponentTrack(NewPCBLibComp, -7, -3.45, 7, -3.45, eMechanical7, 0.05, False);
        CreateComponentTrack(NewPCBLibComp, 7, -3.45, 7, 3.45, eMechanical7, 0.05, False);
        CreateComponentTrack(NewPCBLibComp, 7, 3.45, -7, 3.45, eMechanical7, 0.05, False);
        CreateComponentTrack(NewPCBLibComp, -7, 3.45, -7, -3.45, eMechanical7, 0.05, False);

        STEPmodel := PcbServer.PCBObjectFactory(eComponentBodyObject, eNoDimension, eCreate_Default);
        Model := STEPmodel.ModelFactory_FromFilename('C:\Users\Korisnik\Desktop\FESB Racing\Components\Script\VARRBO750W80L1350T640H1650.STEP', false);
        STEPModel.Layer := eMechanical1;
        STEPmodel.Model := Model;
        STEPmodel.SetState_Identifier('VARRBO750W80L1350T640H1650');
...
Begin
    ...
    CreateComponentVARRBO750W80L1350T640H1650(0);
